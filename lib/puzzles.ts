// Puzzle definitions for Hello Quantum, ported from the Godot "Hello Qubits" puzzles.gd.
// Each entry: title, desc, init (list of [gate,qubit] pairs applied at load),
// goal (Pauli -> target value, empty = no auto-win), gates (allowed-uses structure),
// visible ("all" or list of Pauli keys to show).
// In gates dicts, 0 = unlimited uses; N > 0 = exactly N uses.

export type GatePair = [string, string];
